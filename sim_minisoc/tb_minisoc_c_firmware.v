...
module tb_minisoc_c_firmware;
    ...
    wire [31:0]  core_pc_debug = dut.top_soc_inst.rv32i_core.fetch_stage_inst.pc; 
    ...
    always @(core_pc_debug) begin
        case (core_pc_debug)
            SYM__START:    $display("[FW_MON] 🚀 Boot sequence started (<_start>) at %t", $time);
            SYM_BSS_LOOP:  $display("[FW_MON] 🧹 Initializing .bss section (<bss_loop>) at %t", $time);
            
            // Grâce au nouveau script, ce symbole existe maintenant
            SYM_DATA_LOOP: $display("[FW_MON] 💾 Copying .data section (<data_loop>) at %t", $time);
            
            SYM_MAIN: begin
                $display("[FW_MON] ✅ Jump to main() application at %t", $time);
                // Déclencher la vérification mémoire quand on arrive au main
